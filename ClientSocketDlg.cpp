...
void CClientSocketDlg::OnBtnConnect() 
{
	...
	if (bSuccess && m_SocketManager.WatchComm())
	{
		...
		if (m_nSockType == SOCK_TCP)
		{
			...
			strMsg = _T("Welcome to Server!") + CString("\r\n") + _T("Your ID Number is: (Place ID Number)") + CString("\r\n");
			//sleep_until(system_clock::now() + seconds(1));
			//Sleep(2000);
			strMsg += _T("Please Send Your Number to Server:")+ CString("\r\n");
...
void CClientSocketDlg::OnBtnSend() 
{
	...
	//m_ctlMessage.GetWindowText(strMsg2);
	int nLen = strText.GetLength();
	//int nLen2 = strMsg2.GetLength();
	if (nLen > 0)
	{
		strText += _T("\r\n");
		//strMsg2 += _T("\r\n");
		...
		//nLen2 = strMsg2.GetLength() + 1;
		USES_CONVERSION;
		strcpy_s(reinterpret_cast<LPSTR>(byBuffer), 256, T2CA(strText));
		//strcpy_s(reinterpret_cast<LPSTR>(byBuffer), 256, T2CA("test"));
		m_SocketManager.WriteComm( byBuffer, nLen, INFINITE);
		//m_SocketManager.WriteComm(byBuffer2, nLen2, INFINITE);
