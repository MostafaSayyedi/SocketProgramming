...
void CServerSocketDlg::OnBtnSend() 
{
	CString strText;
	CString strText2;
	//m_pCurServer->AppendMessage("Your ID Number is: ");
			//multiple with StudentNumber
	//int shomaredaneshjoyi = '';
	int result;
	int SNumber = atoi(strText);
	//result = shomaredaneshjoyi * SNumber;
	CString strMsg3;
	//strMsg3.Format("%5", result);
	m_pCurServer->AppendMessage(strMsg3);
	m_ctlMessage.GetWindowText( strText );

	//m_ctlMessage.GetWindowText(strText2);
	int nLen = strText.GetLength();
	//int nLen2 = strText2.GetLength();
	stMessageProxy msgProxy;
	//stMessageProxy msgProxy2;
	if (nLen > 0)
	{
		USES_CONVERSION;
		strText += _T("\r\n");
		...
		else
		{
			...
			/*if (isdigit(SNumber))
			{
				m_pCurServer->AppendMessage("ok its number!");
			}*/
			//memcpy(msgProxy.byData, T2CA("testtt"), nLen);
		}
